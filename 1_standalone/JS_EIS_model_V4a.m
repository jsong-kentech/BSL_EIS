function [output] = JS_EIS_model_V4a()
...
% [v4a] (1) vector-based calculation 
% [v4b] (2) trying particle size distribution

addpath('interpolations\')


% Operation Parameters
clear; clc; close all;
tic;
Ini_Freq = 1e-4; % Don not go more that 10^5, because of the data manipulation made
Fin_Freq = 1e4; 
N = 81;

SOC =  48;    % state of charge of the full cell
SOC = SOC/100;
T = 298.15;                        % {modified} - used for evaluating Ds and i0
...
Dsa = Dsa_function(theta0a,T);        Dsc = Dsc_function(theta0c,T); % {modified} [m^2/sec]      
% i0refa = 3.75;                      i0refc = 10;
bruga = 1.44;                       brugc = 1.44;   % {modified}

% [v6a] particle size distribution
mean_ra = 1;                        mean_rc = 1;
std_ra = 0.3;                       std_rc = 0.65;      % thse are parameters for observed lognormal distribution, dimensionless
...
kc = kc_function(theta0c,T,ctc); % {modified}
...
dUdcc = (1/ctc)*(Uc_function_v2(theta0c+dx,chg) - Uc_function_v2(theta0c-dx,chg))/(2*dx);   % {modified}
dUdca = (1/cta)*(Ua_function_v2(theta0a+dx,chg) - Ua_function_v2(theta0a-dx,chg))/(2*dx);
...
(epssc/tauc)
%}
omegagen=2*pi*logspace(log10(Ini_Freq),log10(Fin_Freq),N); % frequency spectrum [rad/sec]



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% [v6a] eliminated for-loop for vector-based calculation.
%*************************************************************************
% There are three types of dimensionless laplace variables defined in each
% region

%sa/sc - corresponds to the anode/cathode%      
sa=1i*omegagen*epsla*La^2/Dieffa;          %[refer to list of symbols]        vec
sc=1i*omegagen*epslc*Lc^2/Dieffc;          %[refer to list of symbols]        vec         
ss=1i*omegagen*epsls*Lsep^2/Dieffs;        %[refer to list of symbols]        vec

%s2a/s2c - corresponds to the anode/cathode particle
spa=1i*omegagen;      %   s2a                 %[refer to list of symbols]        vec
spc=1i*omegagen;      %   s2c                 %[refer to list of symbols]        vec

%*************************************************************************

Rcta=R*T/i0a/F/(alphaa+alphac);                 %[refer to list of symbols]        sca
Rctc=R*T/i0c/F/(alphaa+alphac);                 %[refer to list of symbols]        sca

Rdifa=-dUdca*Rpa/Dsa/F;                         %[refer to list of symbols]        sca
Rdifc=-dUdcc*Rpc/Dsc/F;                         %[refer to list of symbols]        sca

Ysa=(sqrt(spa*Rpa^2/Dsa)-tanh(sqrt(spa*Rpa^2/Dsa)))./tanh(sqrt(spa*Rpa^2/Dsa));  %[refer to list of symbols]        vec
Ysc=(sqrt(spc*Rpc^2/Dsc)-tanh(sqrt(spc*Rpc^2/Dsc)))./tanh(sqrt(spc*Rpc^2/Dsc));  %[refer to list of symbols]        vec

zetaa = spa*Cdla + 1./(Rcta+Rdifa./Ysa);          %[Eqn [9] in the paper] vec
zetac = spc*Cdlc + 1./(Rctc+Rdifc./Ysc);          %[Eqn [9] in the paper] vec


%New Beta based on Meyers Case 2
betaa = 1/F*(spa * C_filma + 1./(1./zetaa+Rfa));  % edit -GS  [Eqn[8] in the paper] vec
betac = 1/F*(spc * C_filmc + 1./(1./zetac+Rfc));  % edit -GS  [Eqn[8] in the paper] vec
...
B1a = (1-tplus)*La/F/Dieffa*(2*R*T*(1 - tplus)/F*(1/c_e)*(1+dlnfdlnc))./(1/La/aa/F./betaa);    % {modified} to use dlnf/dlnc    vec
B1c = (1-tplus)*Lc/F/Dieffc*(2*R*T*(1 - tplus)/F*(1/c_e)*(1+dlnfdlnc))./(1/Lc/ac/F./betac);    % {modified} to use dlnf/dlnc    vec   

B2a = La*(1/sigmaeffa+1/kappaeffa)./(1/La/aa/F./betaa);   %[refer to list of symbols]        vec
B2c = Lc*(1/sigmaeffc+1/kappaeffc)./(1/Lc/ac/F./betac);   %[refer to list of symbols]        vec

B3 = 2*kappaeffs*R*T*(1 - tplus)/Dieffs/F*(1/c_e)*(1+dlnfdlnc);  % {modified} to use dlnf/dlnc        sca

%EIGEN VALUES

lambda1a = 1/2*(sa + B1a + B2a + sqrt(sa.^2 + 2*B1a.*sa - 2*sa.*B2a + B1a.^2 + 2* B1a.*B2a + B2a.^2));    % [Eqn [5]in paper]   vec
lambda1c = 1/2*(sc + B1c + B2c + sqrt(sc.^2 + 2*B1c.*sc - 2*sc.*B2c + B1c.^2 + 2* B1c.*B2c + B2c.^2));    % [Eqn [5]in paper]   vec
 
lambda2a = 1/2*(sa + B1a + B2a - sqrt(sa.^2 + 2*B1a.*sa - 2*sa.*B2a + B1a.^2 + 2*B1a.*B2a + B2a.^2));     % [Eqn [5]in paper]   vec
lambda2c = 1/2*(sc + B1c + B2c - sqrt(sc.^2 + 2*B1c.*sc - 2*sc.*B2c + B1c.^2 + 2*B1c.*B2c + B2c.^2));     % [Eqn [5]in paper]   vec
%----------------------------------------------<<<<<<<<<<< SEPARATOR >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>------------------------------------------------------------%
...
c_imp = -(phi1x1c-phi2_sep_xs_1)/iapp;
a_imp = -(phi2_sep_xs_0-phi1x1a)/iapp;
s_imp = -(phi2_sep_xs_1-phi2_sep_xs_0)/iapp;
fc_imp = -(phi1x1c-phi1x1a)/iapp;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% [v6a] vector-based calculation
...
% Bode plots
w_vector = omegagen/(2*pi()); % into [Hz] from [rad]
figure(2); hold on;
subplot(2,1,1)
